...
<div class="container-fluid">
    <div class="bg-dashboard">
        <h1 class="mt-4 mb-4"><i class="fas fa-box"></i> Data Jenis Barang</h1>
    </div>

    <div class="card mb-4 shadow-sm">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="mb-0"><i class="fas fa-info-circle"></i> Data Jenis Barang</h5>
            <button type="button" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#formModal">
                <i class="fas fa-plus mr-1"></i> Tambah Jenis Barang
            </button>
        </div>
            ...
            <div class="table-responsive">
                <table class="table table-borderless table-sm" id="dataTable" cellspacing="0">
                    ...
                    <tbody>
                        <?php
                        $no = 1;
                        $sql = $conn->query("SELECT * FROM jenisbarang");
                        while ($data = $sql->fetch_assoc()) {
                        ?>
                            <tr>
                                <td data-label="No"><?php echo $no++; ?></td>
                                <td data-label="Nama Jenis"><?php echo $data['nama_jenis'] ?></td>
                                <td data-label="Aksi">
                                    <button type="button" class="btn btn-warning btn-xs" data-toggle="modal" data-target="#formModal-edit<?php echo $data['id_jenis']; ?>" title="Edit">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <a href="javascript:void(0);" class="btn btn-danger btn-xs" onclick="hapusJenis('<?php echo $data['id_jenis']; ?>')" title="Hapus">
                                        <i class="fas fa-trash"></i>
                                    </a>
                                </td>
                                <?php require 'editjenis.php'; ?>
                            </tr>
                        <?php } ?>
                    </tbody>
                </table>
                <?php require 'tambahjenis.php'; ?>
            </div>
        </div>
    </div>
</div>
