<style>
    ...
    @media (max-width: 767.98px) {
        ...
        /* Badge status untuk Tanggal Masuk */
        #dataTable td[data-label="Tanggal Masuk"] .badge-info {
            background: #fbbf24;
            color: #92400e;
            padding: 6px 12px;
            border-radius: 20px;
        ...
        /* Style untuk nama supplier dan user */
...
<div class="container-fluid">
    <div class=" bg-dashboard">
        <h1 class="mt-4 mb-4"><i class="fas fa-box-open"></i> Transaksi Barang Masuk</h1>
    </div>
                    ...
                    <tbody>
                        ...
                        $sql = $conn->query("
                            SELECT 
                                ...
                                MAX(bm.tanggal_masuk) AS tanggal_masuk,
                                MAX(u.username) AS username,
                                MAX(bm.id_masuk) AS id_masuk
                            FROM barangmasuk bm
                            ...
                            GROUP BY bm.kode_masuk
                            ORDER BY MAX(bm.id_masuk) DESC
                        ");
                        ...
                        ?>
                            <tr style="cursor: pointer;" >
                                ...
                                <td data-label="Kode Transaksi" style="color:#2980b9; font-weight: 600; text-decoration:none;">
                                    <a href="?page=detailmasuk&kode=<?= $data['kode_masuk']; ?>" style="color:#2980b9; text-decoration:none;">
                                        <?= $data['kode_masuk']; ?>
                                    </a>
                                </td>
                                ...
                                <td data-label="Nama Supplier"><?= htmlspecialchars($data['nama_supplier']); ?></td>
                                <td data-label="Keterangan"><?= htmlspecialchars($data['keterangan']); ?></td>
                                <td data-label="Tanggal Masuk">
                                    <span class="badge-info"><?= date('d-m-Y', strtotime($data['tanggal_masuk'])); ?></span>
                                </td>
                                <td data-label="User"><?= htmlspecialchars($data['username']); ?></td>
                                <td data-label="Aksi">
                                    <!-- Kalau tombol edit dan hapus jangan sampai ikut klik ke detail -->
                                    <button type="button" class="btn btn-warning btn-xs" data-toggle="modal" data-target="#formModal-edit-multi-<?= $data['kode_masuk']; ?>" title="Edit">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <a href="javascript:void(0)" class="btn btn-danger btn-xs" onclick=" confirmHapus('<?= $data['kode_masuk']; ?>')" title="Hapus">
                                        <i class="fas fa-trash"></i>
                                    </a>
                                </td>
                                <?php require 'editmasuk.php'; ?>
                            </tr>
                        <?php } ?>
                    </tbody>
                </table>
                <?php require 'tambahmasuk.php'; ?>
            </div>
        </div>
    </div>
</div>
...
<script>
    function confirmHapus(kode_masuk) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href = `?page=barangmasuk&aksi=hapusmasuk&kode_masuk=${kode_masuk}`;
            }
        });
    }
</script>